  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-500 text-sm mt-0.5">Every saving and benefit we've found for you</p>
      </div>

      {/* Summary cards */}
      <div className="grid gap-4 sm:grid-cols-3 animate-fade-up">
        <div className="card p-5">
          <p className="text-xs font-semibold uppercase tracking-wider text-gray-400 mb-1">Total potential</p>
          <p className="text-3xl font-extrabold text-gray-900 tabular-nums">{formatCurrency(totalAnnual)}</p>
          <p className="text-xs text-gray-400 mt-0.5">per year across all categories</p>
        </div>
        <div className="card p-5 border-green-100 bg-green-50/50">
          <p className="text-xs font-semibold uppercase tracking-wider text-green-500 mb-1">Already claimed</p>
          <p className="text-3xl font-extrabold text-green-600 tabular-nums">{formatCurrency(totalClaimed)}</p>
          <p className="text-xs text-green-400 mt-0.5">{claimedCount} saving{claimedCount !== 1 ? 's' : ''} actioned</p>
        </div>
              ...
              <CardBody className="space-y-4">
                <div>
                  ...
                  <div className="flex justify-between mt-1.5">
                    <span className="text-xs text-gray-400">{formatCurrency(totalClaimed)} claimed</span>
                    <span className="text-xs text-gray-400">{formatCurrency(totalAnnual)} total</span>
                  </div>
                </div>
                  ...
                  return (
                    <div key={t.category}>
                      <div className="flex items-center justify-between mb-1.5">
                        ...
                        <span className="text-xs text-gray-500 tabular-nums">{formatCurrency(t._sum.annualSaving)}/yr</span>
                      </div>

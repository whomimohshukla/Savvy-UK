...
const CATEGORY_CONFIG: Record<string, { label: string; emoji: string; badge: 'green' | 'amber' | 'blue' | 'purple' | 'pink' | 'gray' }> = {
  ...
  energy:    { label: 'Energy',    emoji: '⚡', badge: 'green'  },
  broadband: { label: 'Broadband', emoji: '📡', badge: 'green'  },
  insurance: { label: 'Insurance', emoji: '🛡️', badge: 'green'  },
  mobile:    { label: 'Mobile',    emoji: '📱', badge: 'green'  },
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-green-600 text-sm mt-0.5">Every saving and benefit we've found for you</p>
      </div>

      {/* Summary cards */}
      <div className="grid gap-4 sm:grid-cols-3 animate-fade-up">
        <div className="card p-5">
          <p className="text-xs font-semibold uppercase tracking-wider text-green-400 mb-1">Total potential</p>
          <p className="text-3xl font-extrabold text-green-950 tabular-nums">{formatCurrency(totalAnnual)}</p>
          <p className="text-xs text-green-400 mt-0.5">per year across all categories</p>
        </div>
        ...
        <div className="card p-5 border-emerald-200 bg-emerald-50">
          <p className="text-xs font-semibold uppercase tracking-wider text-emerald-600 mb-1">Still to claim</p>
          <p className="text-3xl font-extrabold text-emerald-700 tabular-nums">{formatCurrency(pending)}</p>
          <p className="text-xs text-emerald-500 mt-0.5">waiting for you to act</p>
        </div>
      </div>
                  ...
                  {totals.map((t: any) => {
                    ...
                    return (
                      <div key={t.category} className="flex items-center gap-1.5">
                        <span>{cfg.emoji}</span>
                        <span className="text-xs text-green-600 capitalize">{t.category}</span>
                        <span className="text-xs font-semibold text-green-900 tabular-nums">
              ...
              <CardBody className="space-y-4">
                <div>
                  ...
                  <div className="flex justify-between mt-1.5">
                    <span className="text-xs text-green-400">{formatCurrency(totalClaimed)} claimed</span>
                    <span className="text-xs text-green-400">{formatCurrency(totalAnnual)} total</span>
                  </div>
                </div>
                    ...
                    <div key={t.category}>
                      <div className="flex items-center justify-between mb-1.5">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-xs font-medium text-green-800 capitalize">{t.category}</span>
                        </div>
                        <span className="text-xs text-green-500 tabular-nums">{formatCurrency(t._sum.annualSaving)}/yr</span>
                      </div>
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-0.5 flex-wrap">
                      <p className="text-sm font-medium text-green-900">{r.description}</p>
                    ...
                    <p className="text-xs text-green-400">{formatDate(r.createdAt)}</p>
                  </div>
